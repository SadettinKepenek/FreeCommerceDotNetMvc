...
{
    public class OrderMasterManager:IDBOperations<OrderMaster>, IDisposable
    {
        ...
        public int Add(OrderMaster entry)
        {
            using (SqlCommand command = new SqlCommand("sp_ordermaster_insert"))
            {
                ...
                return Utilities.ExecuteCommand<OrderMaster>(sqlCommand);
            }
        }
        ...
        public List<OrderMaster> GetByIntegerKey(int id, string tbl, string key)
        {
            return Utilities.GetByIntegerKey<OrderMaster>(id, tbl, key);
