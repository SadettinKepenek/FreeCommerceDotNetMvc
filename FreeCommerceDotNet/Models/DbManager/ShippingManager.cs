...
{
    public class ShippingManager:IDBOperations<Shipping>, IDisposable
    {
        ...
        public Shipping Get(int id)
        {
            ...
            using (SqlCommand command = new SqlCommand(sqlQuery))
            {
                ...
                var shippings = new List<Shipping>();
                Utilities.ExecuteCommand<Shipping>(sqlCommand, SqlCommandTypes.Select, ref shippings);
                return shippings.FirstOrDefault();
            }
        }

        public int Add(Shipping entry)
        {
            using (SqlCommand command = new SqlCommand("sp_shipping_insert"))
            {
                ...
                return  Utilities.ExecuteCommand<Shipping>(sqlCommand);
        ...
        public bool Delete(int id)
        {
            ...
            using (SqlCommand command = new SqlCommand(sqlQuery))
            {
                var sqlCommand = command;
                sqlCommand.Parameters.AddWithValue("@Id", id);
                return Utilities.ExecuteCommand<Shipping>(sqlCommand, SqlCommandTypes.Remove);
            }
        }
